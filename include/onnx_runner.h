...
struct ArgMaxWithConfResult {
    ...
    std::vector<float> conf; // per-timestep confidence of argmax class: softmax(top1) in [0,1]
};
...
// Like ChayModelVaLayArgMax, but also returns per-timestep confidence computed from logits.
